...
export class PaymentService {
    /* =========================================
     PUBLIC ENTRY POINT
  ========================================= */
    static async initializePayment(orderId, userId, method) {
        if (!orderId || !method)
    ...
    /* =========================================
     FAKE PAYMENT
  ========================================= */
    static async initializeFakePayment(orderId, userId) {
    ...
    static async confirmFakePayment(reference, userId) {
  ...
  ========================================= */
    static async initializePaystackPayment(orderId, userId) {
        const order = await Order.findById(orderId);
        if (!order)
            throw createHttpError(404, 'Order not found');
        if (order.userId?.toString() !== userId)
  ...
  ========================================= */
    static async handlePaystackWebhook(rawBody, signature) {
        const hash = crypto
  ...
  ========================================= */
    static async applyPayment(reference, userId, session) {
        ...
        for (const item of order.products) {
            ...
            }, { $inc: { stocks: -item.quantity } }, { new: true, ...sessionOptions });
            if (!product) {
